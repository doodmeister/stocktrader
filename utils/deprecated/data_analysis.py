...
def load_df(uploaded_file) -> pd.DataFrame:
    ...
    try:
        ...
        if len(df) > 0:
            ...
            # ENHANCED: Convert date/time columns to datetime
            for col in df.columns:
                if col.lower() in ('date', 'datetime', 'time', 'timestamp'):
                    try:
                        ...
                        logger.warning(f"⚠️ Failed to convert {col} to datetime: {e}")
            
            # ENHANCED: Auto-detect potential numeric columns
            numeric_cols = []
            required_cols = ['open', 'high', 'low', 'close', 'volume']
            
            # First, ensure df is a valid DataFrame with columns attribute
            if not isinstance(df, pd.DataFrame) or not hasattr(df, 'columns'):
                logger.error("Invalid DataFrame: df.columns is not iterable")
                return pd.DataFrame()
                
            # First, ensure OHLCV columns are included
            for col in required_cols:
                if col in df.columns:
                    numeric_cols.append(col)
            
            # Then auto-detect other potential numeric columns
            for col in df.columns:
                if col not in numeric_cols:
                    # Try to determine if column contains numeric data
                    try:
                        # Check if column has numeric values by sampling
                        sample = df[col].dropna().head(10)
                        if len(sample) > 0:
                            # Try converting sample to float to see if it works
                            if pd.to_numeric(sample, errors='coerce').notna().all():
                                numeric_cols.append(col)
                                logger.info(f"🔍 Auto-detected numeric column: {col}")
                    except Exception:
                        pass
            
            # ENHANCED: More robust numeric conversion with better error handling
            for col in numeric_cols:
                try:
                    # Convert to numeric and replace strings with NaN
                    original_values = df[col].copy()
                    df[col] = pd.to_numeric(df[col], errors='coerce')
                    
                    # Log the number of values that were coerced to NaN
                    nan_count = df[col].isna().sum() - original_values.isna().sum()
                    if nan_count > 0:
                        logger.warning(f"⚠️ {nan_count} values in '{col}' were non-numeric and converted to NaN")
                    else:
                        logger.info(f"✅ Successfully converted '{col}' to numeric")
                        
                    # Verify conversion was successful by checking for any remaining string values
                    if df[col].dtype == 'object':
                        logger.warning(f"⚠️ Column '{col}' still has object type after conversion attempt")
                        
                except Exception as e:
                    logger.warning(f"⚠️ Failed to convert {col} to numeric: {str(e)}")
            ...
            non_numeric_cols = [col for col in ohlcv_cols if not pd.api.types.is_numeric_dtype(df[col])]
            
            if non_numeric_cols:
                logger.warning(f"⚠️ These columns should be numeric but aren't: {non_numeric_cols}")
                # Force conversion of any non-numeric OHLCV columns - critical for analysis
                for col in non_numeric_cols:
                    df[col] = pd.to_numeric(df[col], errors='coerce')
                    logger.info(f"🛠️ Forced numeric conversion for {col}")
                
                # Check if any contain NaNs after conversion
                nan_counts = {col: df[col].isna().sum() for col in ohlcv_cols if df[col].isna().any()}
                if nan_counts:
                    logger.warning(f"⚠️ NaN values found after conversion: {nan_counts}")
                    st.warning(f"⚠️ Some numeric values could not be parsed: {nan_counts}")
            
            logger.info(f"🎯 Final result: {len(df)} rows, {list(df.columns)}")
            
            # ENHANCED: Final validation of numeric columns to prevent comparison errors
            # Don't let DataFrame leave this function with string values in numeric columns
            for col in ohlcv_cols:
                # Double-check numeric conversion happened properly
                if not pd.api.types.is_numeric_dtype(df[col]):
                    logger.warning(f"⚠️ Final check: Column {col} is still not numeric; forcing conversion")
                    df[col] = pd.to_numeric(df[col], errors='coerce')
...
def get_pattern_results(df: pd.DataFrame, patterns: Tuple[str, ...]) -> List[Dict[str, Any]]:
    """Detect and return selected candlestick patterns with enhanced type safety."""
    results: List[Dict[str, Any]] = []
    
    # Validate inputs to prevent type comparison issues
    if not isinstance(df, pd.DataFrame) or df.empty:
        logger.warning("Invalid or empty DataFrame provided to get_pattern_results")
        return results
        
    if not patterns:
        logger.info("No patterns specified for detection")
        return results
    
    # Ensure OHLC columns are numeric before pattern detection
    required_cols = ['open', 'high', 'low', 'close']
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        logger.error(f"Missing required OHLC columns for pattern detection: {missing_cols}")
        return results
        
    # Create pattern detector with proper initialization
    try:
        pattern_detector = create_pattern_detector()
    except Exception as e:
        logger.error(f"Failed to create pattern detector: {e}")
        return results
    
    # Process each window in the dataframe
    for i in range(len(df)):
        try:
            # Create window with bounds checking
            start_idx = max(0, i-4)  # Look back up to 4 bars
            if start_idx >= len(df) or i >= len(df):
                continue
            ...
            # Ensure window has numeric data types
            for col in required_cols:
                if col in window.columns and not pd.api.types.is_numeric_dtype(window[col]):
            ...
            try:
                ...
                for p in detected:
                    if p in patterns:
                        # Ensure index is an integer and date is properly formatted
                        pattern_index = int(i)
                        try:
                            pattern_date = df.index[i] if hasattr(df.index, 'name') else i
                            if isinstance(pattern_date, pd.Timestamp):
                                pattern_date = pattern_date.strftime('%Y-%m-%d %H:%M')
                            else:
                                pattern_date = str(pattern_date)
                        except Exception as e:
                            logger.warning(f"Error formatting pattern date: {e}")
                            pattern_date = str(i)
                            
                        results.append({
                            "index": pattern_index,
                            "pattern": str(p),  # Ensure pattern is a string
                            "date": pattern_date
                        })
            except Exception as e:
                logger.warning(f"Pattern detection failed at index {i}: {e}")
                continue
    ...
    logger.info(f"Pattern detection complete: found {len(results)} patterns")
    return results
...
def plot_candlestick_with_patterns(df: pd.DataFrame, pattern_results: List[Dict[str, Any]], width: int = 1400, height: int = 800):
    ...
    # ENHANCED: Validate dataframe is a valid pandas DataFrame
    if not isinstance(df, pd.DataFrame):
        logger.error("Invalid DataFrame provided to plot_candlestick_with_patterns")
        st.error("Cannot display chart: Invalid data format")
        return
        
    if df.empty:
        logger.warning("Empty DataFrame provided to plot_candlestick_with_patterns")
        st.warning("Cannot display chart: No data available")
        return

    # ENHANCED: Validate dataframe has required columns, with robust error handling
    required_cols = ['open', 'high', 'low', 'close']
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        logger.error(f"Missing required columns for candlestick chart: {missing_cols}")
        st.error(f"Can't display candlestick chart: Missing columns {', '.join(missing_cols)}")
        return

    # ENHANCED: Verify that all required columns are numeric
    for col in required_cols:
        if not pd.api.types.is_numeric_dtype(df[col]):
            logger.warning(f"Column {col} is not numeric, converting now for charting")
            try:
                df[col] = pd.to_numeric(df[col], errors='coerce')
            except Exception as e:
                logger.error(f"Failed to convert {col} to numeric: {e}")
                st.error(f"Error preparing candlestick chart: Column {col} data type issue")
                return

    # Use timestamp column if present, else fallback to index
    timestamp_col = None
    if hasattr(df, 'columns'):  # Check df has columns attribute
        for col in df.columns:
            if isinstance(col, str) and col.lower() in ("timestamp", "date", "datetime", "time"):
                timestamp_col = col
                break

    x_vals = df[timestamp_col] if timestamp_col else df.index

    # ENHANCED: Create candlestick chart with error handling
    try:
        ...
        st.error("Could not create candlestick chart due to data formatting issues")
        return
    ...
    if not pattern_results:
        logger.info("No patterns provided for chart")
    else:
        for res in pattern_results:
            try:
                # Make sure we have the required fields
                if 'index' not in res or 'pattern' not in res:
                    logger.warning(f"Pattern result missing required fields: {res}")
                    continue
                
                # Make sure we're working with an integer index
                idx = int(res['index']) if isinstance(res['index'], (int, str)) else 0
                pattern_name = str(res['pattern'])
                
                # Validate index is within bounds
                if 0 <= idx < len(df):  
                    pattern_by_idx[idx].append(pattern_name)
                else:
                    logger.warning(f"Pattern index {idx} out of bounds (0-{len(df)-1})")
            except (TypeError, ValueError, IndexError) as e:
                logger.warning(f"Error processing pattern result: {e}")
    
    # ENHANCED: Add pattern markers with comprehensive validation and error handling
    try:
        # Calculate average candle range for positioning
        avg_range = (df['high'] - df['low']).mean()
        if pd.isna(avg_range):
            logger.warning("Cannot calculate average range, using default")
            avg_range = 1.0
        ...
        for idx, patterns in pattern_by_idx.items():
            try:
                ...
                idx_int = int(idx)
                if not (0 <= idx_int < len(df)):
                    continue
                
                # Calculate position with validation
                offset = 0.02 * avg_range * (len(patterns) - 1)
                
                # Safe access with explicit type conversion
                try:
                    high_value = float(df['high'].iloc[idx_int])
                    y = high_value + offset
                except (ValueError, TypeError) as e:
                    logger.warning(f"Cannot convert high value to float at index {idx_int}: {e}")
                    continue
                
                # Get x-coordinate
                try:
                    x = df[timestamp_col].iloc[idx_int] if timestamp_col else df.index[idx_int]
                except IndexError:
                    logger.warning(f"Invalid index {idx_int} for x-coordinate")
                    continue
                    
                # Add marker with safe values
                ...
                logger.warning(f"Error adding pattern marker at index {idx}: {e}")
                continue
    except Exception as e:
        logger.error(f"Error processing patterns for chart: {e}")
        st.warning("Some patterns could not be displayed due to data formatting issues")

    # ENHANCED: Safe layout update and rendering
    try:
        fig.update_layout(width=width, height=height)
        st.plotly_chart(fig, use_container_width=False)
    except Exception as e:
        logger.error(f"Error rendering candlestick chart: {e}")
        st.error("Failed to render candlestick chart due to an unexpected error")
...
class TechnicalAnalysisDashboard:
    ...
    def run(self):
        ...
        # Enhanced debug logging for all returned values and types
        logger.debug(f"evaluate() returned: signal_value={signal_value} (type={type(signal_value)}), rsi_score={rsi_score} (type={type(rsi_score)}), macd_score={macd_score} (type={type(macd_score)}), bb_score={bb_score} (type={type(bb_score)})")
        # Defensive float cast and logging for all technical signal values
        def safe_float(val, name):
            try:
                fval = float(val)
                logger.debug(f"{name} value for display: {fval} (type={type(fval)})")
                return fval
            except Exception as e:
                logger.error(f"Could not convert {name} to float: {val} ({e})")
                return 0.0

        # Float-cast before any display or comparison
        ...
        # Log float-cast values
        logger.debug(f"Float-cast values: signal_value_float={signal_value_float}, rsi_score_float={rsi_score_float}, macd_score_float={macd_score_float}, bb_score_float={bb_score_float}")

        col1, col2 = st.columns([2, 1])
        with col1:
            try:
                st.metric("Combined Technical Signal", f"{signal_value_float:.3f}", help="Range: -1 (bearish) to 1 (bullish)")
            except Exception as e:
                logger.error(f"st.metric error for Combined Technical Signal: {e} (signal_value_float={signal_value_float}, type={type(signal_value_float)})")
                st.metric("Combined Technical Signal", "N/A", help="Range: -1 (bearish) to 1 (bullish)")
        ...
        with col2:
            with st.expander("Component Scores"):
                try:
                    st.metric("RSI", f"{rsi_score_float:.2f}" if rsi_score_float is not None else "N/A")
                except Exception as e:
                    logger.error(f"st.metric error for RSI: {e} (rsi_score_float={rsi_score_float}, type={type(rsi_score_float)})")
                    st.metric("RSI", "N/A")
                try:
                    st.metric("MACD", f"{macd_score_float:.2f}" if macd_score_float is not None else "N/A")
                except Exception as e:
                    logger.error(f"st.metric error for MACD: {e} (macd_score_float={macd_score_float}, type={type(macd_score_float)})")
                    st.metric("MACD", "N/A")
                try:
                    st.metric("BB", f"{bb_score_float:.2f}" if bb_score_float is not None else "N/A")
                except Exception as e:
                    logger.error(f"st.metric error for BB: {e} (bb_score_float={bb_score_float}, type={type(bb_score_float)})")
                    st.metric("BB", "N/A")
        ...
        with col2:
            ...
            ta = TechnicalAnalysis(df)
            try:
                pt = float(ta.calculate_price_target())
            except (TypeError, ValueError) as e:
                logger.error(f"Error converting price target to float: {e}")
                pt = None
        ...
        with col3:
            ...
            ta = TechnicalAnalysis(df)
            try:
                pt_fib = float(ta.calculate_price_target_fib(lookback=fib_lookback_int, extension=fib_ext_float))
            except (TypeError, ValueError) as e:
                logger.error(f"Error converting fibonacci price target to float: {e}")
                pt_fib = None
        ...
        try:
            ...
            end_date = str(df[timestamp_col].iloc[-1])        # Ensure all values are float before using in f-strings
        try:
        ...
        try:
            rsi_latest = float(rsi.iloc[-1]) if not rsi.isna().all() and not pd.isna(rsi.iloc[-1]) else None
        except:
            rsi_latest = None
            
        try:
            macd_latest = float(macd_line.iloc[-1]) if not macd_line.isna().all() and not pd.isna(macd_line.iloc[-1]) else 0.0
            signal_latest = float(signal_line.iloc[-1]) if not signal_line.isna().all() and not pd.isna(signal_line.iloc[-1]) else 0.0
        except:
            macd_latest = None
            signal_latest = None
            
        try:
            bb_upper_latest = float(upper_band.iloc[-1]) if not upper_band.isna().all() and not pd.isna(upper_band.iloc[-1]) else 0.0
            bb_lower_latest = float(lower_band.iloc[-1]) if not lower_band.isna().all() and not pd.isna(lower_band.iloc[-1]) else 0.0
        except:
            bb_upper_latest = None
            bb_lower_latest = None
            
        summary_lines: List[str] = [
            f"Technical Analysis Summary for {stock_ticker}:",
            f"CSV File: {filename}",
            f"Date Range: {start_date} to {end_date}",
            f"Latest Close: {latest_close:.2f}" if latest_close is not None else "Latest Close: N/A",
            f"RSI (period={rsi_period}): {rsi_latest:.2f}" if rsi_latest is not None else "RSI: N/A",
            f"MACD ({macd_fast}/{macd_slow}): {macd_latest:.2f}, Signal: {signal_latest:.2f}" if macd_latest is not None and signal_latest is not None else "MACD: N/A",
            f"Bollinger Bands ({bb_period}±{bb_std}): Upper={bb_upper_latest:.2f}, Lower={bb_lower_latest:.2f}" if bb_upper_latest is not None and bb_lower_latest is not None else "Bollinger Bands: N/A",
            f"ATR: {atr_value:.3f}" if atr_value is not None else "ATR: N/A",
            f"Price Target: {pt:.3f}" if pt is not None else "Price Target: N/A",
            f"Combined Signal: {signal_value_float:.3f}" if signal_value_float is not None else "Combined Signal: N/A",
            "",
            "Detected Patterns (last 20):"
        ]
        if patterns:
            for p in patterns[-20:]:
                # Make sure index and date are properly formatted
                p_index = p['index']
                p_date = p['date']
                if not isinstance(p_date, str):
                    p_date = str(p_date)
                summary_lines.append(f"- Index {p_index}, Date {p_date}: {p['pattern']}")
        else:
