...
def load_df(uploaded_file) -> pd.DataFrame:
    ...
    try:
        # Try different parsing approaches for CSV files
        # First attempt: standard parsing
        df = pd.read_csv(uploaded_file)
        
        # If only 1 row (header only), try different separators
        if len(df) <= 1:
            uploaded_file.seek(0)  # Reset file pointer
            
            # Try semicolon separator (common in European CSVs)
        ...
        # Log successful loading before processing
        logger.info(f"Raw CSV loaded: {len(df)} rows, {len(df.columns)} columns")
        logger.debug(f"Original columns: {list(df.columns)}")
        
        # Normalize column names (case-insensitive and handle variations)
        df.columns = df.columns.str.lower().str.strip()
        
        # Handle common date/timestamp column variations
        ...
        # If we found a date column, set it as index (optional for analysis)
        if date_col:
            try:
                df[date_col] = pd.to_datetime(df[date_col])
                df.set_index(date_col, inplace=True)
                logger.info(f"Set {date_col} as datetime index")
        ...
        # Final validation
        logger.info(f"Successfully processed CSV: {len(df)} rows, columns: {list(df.columns)}")
